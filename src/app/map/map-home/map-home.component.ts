    ...
    <div class="map-home">
      ...
      <div class="map-home__topbar">
        <div class="map-home__search-box" (click)="goToDestination()">
          <span class="map-home__search-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" width="18" height="18"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></span>
      ...
      <button class="map-home__fab map-home__fab--notif" (click)="toggleNotif()">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"/></svg>
      ...
      <button class="map-home__fab map-home__fab--locate" (click)="locateMe()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3"/><circle cx="12" cy="12" r="9" stroke-dasharray="4 2"/></svg></button>
      ...
      <div class="map-home__panel"
        ...
        [style.transition]="panelDragging ? 'none' : 'transform .35s cubic-bezier(.32,.72,0,1)'">
        <div class="map-home__drag-bar"
          (touchstart)="onPanelDragStart($event)"
          (touchmove)="onPanelDragMove($event)"
          (touchend)="onPanelDragEnd()"></div>
        ...
        <div class="map-home__routes">
          <div class="map-home__route-opt map-home__route-opt--best" (click)="goToDetail('r1')">
          ...
          <div class="map-home__route-opt" (click)="goToDetail('r2')">
          ...
          <div class="map-home__route-opt" (click)="goToDetail('r3')">
...
export class MapHomeComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly PANEL_COLLAPSE_HEIGHT = 280; // px to shift down when collapsed
  ...
  private initMap(): void {
    ...
    this.gpsService.requestOnce().then(({ pos, error }) => {
      if (error === 'denied') {
        this.toastMsg = '🔒 Activa ubicación en el navegador para GPS real';
  ...
  goToDetail(routeId: string): void { this.router.navigate(['/map/detail'], { queryParams: { id: routeId } }); }
  toggleNotif(): void { this.showNotif = !this.showNotif; if (this.showNotif) this.notifCount = 0; }

  locateMe(): void {
    if (!this.map) return;
    this.toastMsg = '🔍 Buscando ubicación...';
    this.cdr.detectChanges();
    this.gpsService.requestOnce().then(({ pos, error }) => {
      if (error === 'denied') {
        this.toastMsg = '🔒 Permiso de ubicación denegado — actívalo en el navegador';
      } else if (error === 'timeout') {
        this.toastMsg = '⏱️ GPS tardó demasiado, intenta de nuevo';
      } else if (pos.accuracy > 0) {
        this.map.setView([pos.lat, pos.lng], 16);
        this.updateUserMarker(pos.lat, pos.lng, pos.accuracy);
        this.toastMsg = '📍 Ubicación actualizada';
      } else {
        this.toastMsg = '📍 Usando ubicación: Santo Domingo';
      }
      this.cdr.detectChanges();
  ...
  onPanelDragStart(e: TouchEvent): void {
    this.panelDragging = true;
    this.dragStartY = e.touches[0].clientY;
    this.dragStartOffset = this.panelOffset;
  }

  onPanelDragMove(e: TouchEvent): void {
    if (!this.panelDragging) return;
    const delta = e.touches[0].clientY - this.dragStartY;
    const raw = this.dragStartOffset + delta;
    // Only allow dragging downward (positive offset), with slight resistance past collapse point
    if (raw < 0) {
      this.panelOffset = raw * 0.15; // resist pulling up past open
    } else if (raw > this.PANEL_COLLAPSE_HEIGHT) {
      this.panelOffset = this.PANEL_COLLAPSE_HEIGHT + (raw - this.PANEL_COLLAPSE_HEIGHT) * 0.15;
    } else {
      this.panelOffset = raw;
    }
  }

  onPanelDragEnd(): void {
    this.panelDragging = false;
    const threshold = this.PANEL_COLLAPSE_HEIGHT / 2;
    const isCollapsed = this.dragStartOffset >= this.PANEL_COLLAPSE_HEIGHT * 0.5;
    if (isCollapsed) {
      // Was collapsed — small drag up reopens, otherwise stays collapsed
      this.panelOffset = this.panelOffset < threshold ? 0 : this.PANEL_COLLAPSE_HEIGHT;
    } else {
      // Was open — drag down enough to collapse
      this.panelOffset = this.panelOffset > threshold ? this.PANEL_COLLAPSE_HEIGHT : 0;
    }
    this.cdr.detectChanges();
  }
